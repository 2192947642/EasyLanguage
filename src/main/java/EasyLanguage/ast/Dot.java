...
public class Dot extends Postfix {
        ...
        if (value instanceof ClassInfo ci) {//如果执行者是类的定义信息
            if ("new".equals(member)) {//如果是新建对象
                ...
                if(!(constructor instanceof Function)){
                    constructor=ci.write("constructor",new Function(new ParameterList(new ArrayList<>()),new BlockStatement(new ArrayList<>()),environment,true));
                }
    ...
    protected void initObject(ClassInfo ci, Environment currentEnv) {//初始化对象,也就是将对象的代码块在该环境中进行运行
        if (ci.superClass() != null){//如果存在父类 那么先运行父类的代码块
         ...
         Object constructor = father.read("constructor");
         if(!(constructor instanceof Function)){//将构造方法写入到父类中
             constructor=ci.write("constructor",new Function(new ParameterList(new ArrayList<>()),new BlockStatement(new ArrayList<>()),fartherEnv,true));
         }
         if(constructor instanceof Function function){
             if(function.parameters().size()==0){
                 function.body().eval(fartherEnv);
             }
         }
        }
